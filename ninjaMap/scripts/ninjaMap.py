...
class Strains:
    ...
    def calculate_singular_coverage(self, bamfile_name):
        ...
        for contig_name in self.contigs.keys():
            for pileupcolumn in cov_bamfile.pileup(
                contig=contig_name, stepper="all", min_base_quality=20
            ):
                base_cov_contribution = 0
                if pileupcolumn.nsegments > 0:
                    base_cov_contribution = self._calc_base_depth(
                        pileupcolumn, self.singular_bin
                    )
    ...
    def calculate_escrow_coverage(self, bamfile_name):
        ...
        return (
            ...
            self.total_covered_depth,
            self.weighted_base_depth,
        )  # , self.covered_bases) #mycovered_bases )
